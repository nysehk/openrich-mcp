...
<p align="center">
  <strong>中国 A 股股票与金融数据 MCP Server</strong><br/>
...
**cn-stock-mcp** 是一个遵循 [Model Context Protocol (MCP)](https://modelcontextprotocol.io) 标准的中国 A 股数据服务器。它让任何支持 MCP 的 AI Agent（如 OpenRich Harness、Claude Code、Cursor）直接调用行情、财报、行业、宏观经济等 **42 个金融工具**，无需 API Key，开箱即用。
...
在上游财务报表精简输出基础上，扩展了银行尽调所需的利润表、资产负债表和现金流量表科目，包括利息收支、资产/信用减值、应收应付、租赁负债、递延所得税、所有者权益和现金流明细等，并统一换算为亿元输出。
...
```bash
PYTHONPATH=src python -m cn_stock_mcp --help
```

```
usage: __main__.py [-h] [--http] [--port PORT] [--host HOST]

cn-stock-mcp: China Financial Data MCP Server based on AKShare
...
```json
{
  "mcpServers": {
    "cn-stock": {
      ...
      "args": ["-m", "cn_stock_mcp"],
      "env": {
        "PYTHONPATH": "/absolute/path/to/cn-stock-mcp/src"
      }
    }
  }
}
...
> 将 `/absolute/path/to/cn-stock-mcp` 替换为你的实际路径。配置好后 Agent 会自动拉起 MCP 进程。
...
- [ccq1/cn-financial-mcp](https://github.com/ccq1/cn-financial-mcp) — 本项目的上游基础实现
