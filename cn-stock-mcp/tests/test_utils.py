...
from cn_stock_mcp.utils.cache import TTLCache
from cn_stock_mcp.utils.formatter import (
    BALANCE_SHEET_COLS,
...
from cn_stock_mcp.utils.symbol import (
...
class TestFormatter:
    ...
    @pytest.mark.parametrize(
        ("whitelist", "raw_column", "display_column"),
        [
            (INCOME_STATEMENT_COLS, "FE_INTEREST_EXPENSE", "财务费用-利息费用(亿)"),
            (BALANCE_SHEET_COLS, "FIXED_ASSET", "固定资产(亿)"),
            (CASHFLOW_STATEMENT_COLS, "PAY_DEBT_CASH", "偿还债务支付的现金(亿)"),
        ],
    )
    def test_openrich_extended_financial_fields(
        self, whitelist, raw_column, display_column
    ):
        source = pd.DataFrame(
            {"REPORT_DATE_NAME": ["2025年报"], raw_column: [250_000_000]}
        )
        result = slim_financial_df(source, whitelist)

        assert result.loc[0, display_column] == 2.5
